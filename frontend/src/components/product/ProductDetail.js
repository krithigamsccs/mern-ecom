...
//     const { id } = useParams();
//     const [quantity, setQuantity] = useState(1);
//     const [isOrdered, setIsOrdered] = useState(false); // New state to track order status
...
//         dispatch(addCartItem(product._id, quantity));
//         setIsOrdered(true); // Set isOrdered to true when added to cart
//         toast('Cart Item Added!', {
//             type: 'success',
//             position: toast.POSITION.BOTTOM_CENTER
//         });

//         // Simulate order completion for testing (This should be replaced with real order logic)
//         // setIsOrdered(true);
//     };
...
//                             <button type="button" id="cart_btn"
//                                 disabled={product.stock === 0 ? true : false}
//                                 onClick={handleAddToCart} // Update to use the new function
//                                 className="btn btn-primary d-inline ml-4"
//                             >Add to Cart</button>
...
//                                     disabled={!isOrdered} // Disable if not ordered
//                                 >
//                                     Submit Your Review
//                                 </button> :
//                                 <div className="alert alert-danger mt-5"> Login to Post Review</div>
...
import { toast } from "react-toastify";
import ProductReview from "./ProductReview";
import { userOrders } from "../../actions/orderActions"; // Import the userOrders action

export default function ProductDetail() {
    const { loading, product = {}, isReviewSubmitted, error } = useSelector((state) => state.productState);
    const { user } = useSelector(state => state.authState);
    const { userOrders: orders, loading: ordersLoading, error: ordersError } = useSelector(state => state.orderState); // Access user orders
    const dispatch = useDispatch();
    const { id } = useParams();
    const [quantity, setQuantity] = useState(1);
    const [isOrdered, setIsOrdered] = useState(false); // This can be deprecated
    const [canReview, setCanReview] = useState(false); // New state to track review eligibility
    ...
    useEffect(() => {
        if (orders && orders.length > 0) {
            // Check if any order includes this product and is delivered
                ...
                <Fragment>
                    ...
                    <div className="row f-flex justify-content-around">
                        ...
                        <div className="col-12 col-lg-5 mt-5">
                            ...
                            {user ?
                                <button onClick={handleShow} id="review_btn" type="button" className="btn btn-primary mt-4"
                                    disabled={!canReview} // Disable if not eligible
                                >
                                    Submit Your Review
                                </button> :
                                <div className="alert alert-danger mt-5"> Login to Post Review</div>
                                ...
                                <div className="rating w-50">
                                    <Modal show={show} onHide={handleClose}>
                                        ...
                                        <Modal.Body>
                                            ...
                                            <textarea onChange={(e) => setComment(e.target.value)} name="review" id="review" className="form-control mt-3" placeholder="Write your review here..."></textarea>
